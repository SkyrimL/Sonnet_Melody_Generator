...
original_title = '<p style="font-family:Ariel; text-align:center; color:saddlebrown ; font-size:50px; opacity: 0.9;">Sonnet Melody Generator</p>'
...
if __name__ == '__main__':
    ...
    selectbox_text = '<p style="font-family:Ariel; text-align:left; color:peru; font-size:30px;">Please select the emotion of the song</p>'
    st.sidebar.markdown(selectbox_text, unsafe_allow_html=True)
    option = st.sidebar.selectbox("", ('Happy', 'Sad'))
    ...
    # Parse command line arguments
        ...
        st.subheader('The sonnet is generated as: ')
        st.write(sonnet)
        ...
        for lyrics in entire_lyrics:
            ...
            with tf.Session(graph=tf.Graph()) as sess:
                ...
                mp3_input="melody{0}.mp3".format(index)
                
                print('done', index)
                st.subheader(index)
                st.write('14 in total')
                st.write(sonnet.split('\n')[index-1])
                st.audio(mp3_input,format='audio/mp3')
        ...
        feedback = '<p style="font-family:Ariel; text-align:left; color:saddlebrown ; font-size:20px>Feedback</p>'
        st.markdown(feedback, unsafe_allow_html=True)
        st.selectbox("", ('Your feeling of the song','Happy','Sad' ))
